...
public class Test {
	public static void main(String[] args) {
		...
		System.out.println("(3) 리스트에 첫번째에 노드 추가하기");
		list.insertFirstNode("토");
		list.printList();

		System.out.println("(4) 리스트에 마지막 노드 삭제하기");
		list.deleteLastNode("토");
		list.printList();

		System.out.println("(5) 리스트에 중간 노드\"수\" 삭제하기");
		list.deleteLastNode("수");
		list.printList();

		System.out.println("(6) 리스트에 첫번째 노드 삭제하기");
		list.deleteFirstNode("수");
		list.printList();
...
class LinkedList {
	...
	void deleteLastNode(String string) {
		if (head == null) {
	...
	void deleteFirstNode(String string) {
		if(head ==null) {
			System.out.println("지울 노드가 존재하지 않습니다.");
		}else {
			head = head.next;
	...
	void reverseList(String string) {
		ListNode next = head;
